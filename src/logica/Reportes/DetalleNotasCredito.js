...
async function contarRegistros(fechaInicio, fechaFin, idSucursal, upc, origen) {
    ...
    let tabla = '';
    if (origen === 'megared') tabla = 'notacreditomegared';
    else if (origen === 'bodegona') tabla = 'notacreditobodegonaantigua';
    else if (origen === 'surti') tabla = 'notascreditosurtimayoreo';
...
// ========================================
async function buscarNotasCredito(origen) {
...
async function cargarTodoDeUnaVez(fechaInicio, fechaFin, idSucursal, upc, totalRegistros, origen) {
    ...
    try {
        ...
        if (origen === 'megared') {
            ...
            `;
        } else if (origen === 'bodegona') {
            tabla = 'notacreditobodegonaantigua';
            query = `
                SELECT
                    DATE_FORMAT(Fecha, '%Y-%m-%d') as Fecha,
                    NombreSucursal,
            ...
            tabla = 'notascreditosurtimayoreo';
            query = `
                SELECT
                    DATE_FORMAT(Fecha, '%Y-%m-%d') as Fecha,
                    NombreSucursal,
        ...
        query += ' ORDER BY Fecha ASC, IdNotaCredito DESC';
...
async function exportarExcel() {
    let nombreOrigen = '';
    if (origenActual === 'megared') nombreOrigen = 'NotasCreditoMegared';
    else if (origenActual === 'bodegona') nombreOrigen = 'NotasCreditoBodegonaAntigua';
    else if (origenActual === 'surti') nombreOrigen = 'NotasCreditoSurtiMayoreo';
    ...
    try {
        ...
        const nombreArchivo = `${nombreOrigen}_${fecha}_${Date.now()}.xlsx`;
        ...
        const crearNuevaHoja = () => {
            const nombreHoja = totalHojas > 1 ? `Notas Crédito ${hojaActual}` : nombreOrigen;
            ...
            worksheet.columns = [
                ...
                { header: 'Id Nota Crédito', key: 'IdNotaCredito', width: 18 },
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Botones de búsqueda
    document.getElementById('btnBuscarMegared').addEventListener('click', () => buscarNotasCredito('megared'));
    document.getElementById('btnBuscarBodegona').addEventListener('click', () => buscarNotasCredito('bodegona'));
    document.getElementById('btnBuscarSurti').addEventListener('click', () => buscarNotasCredito('surti'));
